...
#include "tensorflow/lite/experimental/litert/vendors/mediatek/compiler/legalizations/add_legalization.h"
...
Expected<void> LegalizeAdd(const NeuronAdapter& neuron_adapter,
  ...
  constexpr NeuronOperandType int32_scalar_type = {
      .type = NEURON_INT32,
      .dimensionCount = 0,
  };
  auto fused_activation_operand_index = operand_map.Register(int32_scalar_type);
  if (!fused_activation_operand_index) {
    return fused_activation_operand_index.Error();
  }

  int32_t fused_activation_value = tfl_fused_activation;
  if (neuron_adapter.api().model_set_operand_value(
          model, *fused_activation_operand_index, &fused_activation_value,
          sizeof(fused_activation_value)) != NEURON_NO_ERROR) {
